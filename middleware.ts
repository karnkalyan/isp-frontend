...
export async function middleware(request: NextRequest) {
  ...
  const isPublicAsset = /\.(?:avif|css|gif|ico|jpe?g|js|json|map|png|svg|webp|woff2?|ttf|eot)$/i.test(pathname);
  if (isPublicAsset || IGNORED_PREFIXES.some((p) => pathname.startsWith(p))) {
