...
import { Nas } from "./nas-list"
import { useConfirmToast } from "@/hooks/use-confirm-toast"

export function UpdateNasForm({ nasId }: { nasId: string }) {
  ...
  useEffect(() => {
    const loadNasData = async () => {
      try {
        setFetching(true)
        const nas: Nas = await apiRequest(`/nas/${nasId}`)
        
        setFormData({
          nasname: nas.nasname || "",
          shortname: nas.shortname || "",
          type: nas.type || "other",
          ports: nas.ports ? Number(nas.ports) : 1812,
          secret: nas.secret || "",
          server: nas.server || "radius",
          community: nas.community || "public",
          description: nas.description || "",
          isActive: nas.isActive,
          isDefault: nas.isDefault
        })
      } catch (error: any) {
        toast.error(error.message || "Failed to load NAS details")
        router.push("/nas")
      } finally {
        setFetching(false)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirm({
      title: "Update NAS",
      message: `Are you sure you want to update NAS "${formData.shortname}" (${formData.nasname})?`,
      type: "warning",
      confirmText: "Update",
      cancelText: "Cancel",
    })
    ...
    try {
      await apiRequest(`/nas/${nasId}`, { method: 'PUT', body: JSON.stringify(formData) })
      
      toast.success("NAS Configuration updated successfully")
      router.push("/nas")
    } catch (error: any) {
      toast.error(error.message || "Failed to update NAS")
      setLoading(false)
    }
  }
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-muted-foreground">Loading NAS configuration...</p>
      </div>
    )
  }
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end space-x-4">
        <Button variant="outline" type="button" onClick={handleBack} disabled={loading || fetching}>
          Cancel
        </Button>
