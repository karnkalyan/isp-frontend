...
export default function TicketDetailPage() {
  ...
  const typeName = useMemo(() => {
    if (!ticket?.ticketTypeId) return ticket?.category || "Unclassified"
    return ticketTypes.find(t => t.id === ticket.ticketTypeId)?.name || ticket?.category || "Unclassified"
  }, [ticket, ticketTypes])

  const canConvertLead = useMemo(() => {
    if (ticket?.subject?.type !== "LEAD" || String(ticket.subject.status || "").toLowerCase() === "converted") return false
    const classification = `${typeName} ${ticket.category || ""}`.toLowerCase()
    return classification.includes("new connection") || classification.includes("new installation")
  }, [ticket, typeName])
